...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        if (state, action) in self.QValues: # or the Q node value otherwise
            pass
        else:
          self.QValues[(state, action)] = 0.0 # Should return 0.0 if we have never seen a state
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(allowedActions) == 0: #if there are no legal actions
            return 0.0 #should return a value of 0.0
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(legalActions) == 0: # Return "None" if there are no actions
            return None
        actionVals = {}
    ...
    def getAction(self, state):
        ...
        if len(legalActions) == 0:
            return None # Take no action if there are no legal actions
        action = None
        "*** YOUR CODE HERE ***"
        if not util.flipCoin(self.epsilon):
            # Compute action in state
            action = self.getPolicy(state)
        else:
             # Take random action from list
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        oldValue = self.getQValue(state,action) # Update Q value
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        return features * self.weights # Return Q value
