...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        """
        "Q6"
        actions = self.getLegalActions(state)#得到合法的action
        if not actions:#沒找到，return none
          return None
    ...
    def getAction(self, state):
        ...
        action = None
        "Q7"
        if util.flipCoin(self.epsilon):#如果flip coin回傳值為true，執行隨機路徑走法
          action = random.choice(legalActions)
        else:
          action = self.computeActionFromQValues(state)
        return action #走原本的路徑
